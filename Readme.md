...
1.  **Server-Side Pagination & Optimization:**
    ...
    -   *Current State:* The dashboard fetches all user jobs at once.

    -   *Improvement:* If a user has 1,000+ jobs, this becomes slow. I would implement pagination to send jobs in **batches of 5** (sorted by latest). The UI would include "Next" and "Previous" controls to navigate through the history, significantly reducing database load and improving "Time to Interactive."
